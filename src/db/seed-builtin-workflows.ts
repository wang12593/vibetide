 ...
 *
 * 本 Chunk A 交付：脚手架 + 9 条主流水线数字员工预设（xiaolei / xiaoce / xiaozi 各 3）
 ...
 *  - ownerEmployeeId：数字员工专属场景的归属，公共场景为 null
...
export interface BuiltinWorkflowSeed {
  ...
  /** 数字员工专属场景归属；null = 公共场景 */
  ownerEmployeeId: EmployeeId | null;
  /** 默认团队（第一个一般是 owner / 主执行数字员工） */
...
export const BUILTIN_WORKFLOWS: BuiltinWorkflowSeed[] = [
    ...
    inputFields: [
      {
        ...
        type: "text",
        required: true,
        placeholder: "如：技术保障部 2026 春季产品发布会",
      },
      {
        name: "go_live_at",
        label: "开播时间",
...

  {
    ...
    inputFields: [
      ...
      },
      {
        ...
        type: "textarea",
        required: true,
        placeholder: "请详细描述主要先进事迹，包括具体事例和数据",
      },
      {
        name: "honors",
        label: "所获荣誉",
        type: "textarea",
        required: true,
        placeholder: "请列举所获荣誉称号",
      },
      {
        name: "officialEvaluation",
        label: "官方评价",
        type: "textarea",
        required: true,
        placeholder: "请输入官方对该人物的评价原文",
      },
      {
        ...
        type: "textarea",
        required: true,
        placeholder: "请说明文章的宣传主题和重点",
      },
    ],
    systemInstruction:
      "按照解放军报解辛平评论风格创作重要新闻评论文章。素材：人物{{personName}}（{{personTitle}}，{{personUnit}}），事迹{{achievements}}，荣誉{{honors}}，官方评价{{officialEvaluation}}，宣传重点{{propagandaFocus}}。必须包含：1) 素材要素自检（检查必备字段完整性）2) 四部分结构规划（开篇点题/事迹陈述/精神升华/意义阐述）3) 解辛平风格创作（庄重严谨、政治站位高）4) 严格质量审核。",
    promptTemplate:
      "创作解辛平风格文章：人物{{personName}}（{{personTitle}}@{{personUnit}}），事迹{{achievements}}，荣誉{{honors}}，评价{{officialEvaluation}}，宣传重点{{propagandaFocus}}。",
    steps: [
      step(1, "素材提交与要素自检", "task_planning", "任务规划", "analysis", "selfcheck", {
        requiredFields: ["personName", "personTitle", "personUnit", "achievements", "honors", "officialEvaluation", "propagandaFocus"],
      }),
      step(2, "文章结构规划", "topic_extraction", "主题提取", "analysis", "structure", {
        structureType: "jiexping",
        sections: ["opening", "mainBody", "spiritElevation", "conclusion"],
      }),
      step(3, "解辛平风格创作", "content_generate", "内容生成", "generation", "write", {
  ...
  {
    slug: "pub.page_inspection",
    name: "网页内容检测",
    description: "利用浏览器自动化技术检测网页内容错误、页面错误、链接可用性、图片加载等问题。",
    icon: "globe",
    category: "analytics",
    ownerEmployeeId: "xiaolei",
    defaultTeam: ["xiaolei", "xiaozi", "xiaoshen"],
    inputFields: [
      {
        name: "urls",
        label: "待检测网址",
    ...
    systemInstruction:
      "利用浏览器自动化技术检测网页内容。待检测URL：{{urls}}，超时{{timeout}}秒。检测项目：1) 页面加载状态（HTTP状态码/加载时间）2) 链接可用性（内部/外部/锚点链接）3) 图片加载（URL有效性/加载时间/替代文本）4) 内容质量（拼写/语法/完整性）5) SEO基础（标题长度/Meta描述/关键词密度）。产出详细检测报告。",
    promptTemplate:
      "检测网页{{urls}}的内容质量，超时{{timeout}}秒，检查链接{{checkLinks}}，检查图片{{checkImages}}。",
    steps: [
      step(1, "打开目标网页", "browser_automation", "浏览器自动化", "other", "navigate", {
        action: "navigate",
      }),
      step(2, "页面加载检测", "browser_automation", "浏览器自动化", "other", "screenshot", {
        action: "screenshot",
      }),
      step(3, "内容质量分析", "quality_review", "质量审核", "management", "quality", {
        reviewTier: "standard",
        contentType: "article",
      }),
      step(4, "生成检测报告", "data_report", "数据报告", "analysis", "report"),
    ],
  },
  ...
  // ════════════════════════════════════════════════════════════════════════
  // 深度报道制作（交互式演示）· 小乐+小深+小文+小鉴 · 4步骤
  // ════════════════════════════════════════════════════════════════════════
  {
    slug: "deep_report_interactive",
    name: "深度报道制作（交互式）",
    description: "从热点发现、深度调研、内容撰写到质量审核，每个步骤完成后由穆兰向您汇报成果，您可以确认继续或提出修改意见。",
    icon: "file-text",
    category: "deep",
    ownerEmployeeId: null,
    defaultTeam: ["xiaolei", "xiaoshen", "xiaowen", "xiaojian"],
    inputFields: [
      {
        name: "topic",
        label: "报道主题",
      ...
      },
      {
        ...
        placeholder: "选择深度",
        options: [
          { value: "brief", label: "简报（1000字内）" },
          { value: "standard", label: "标准（2000-3000字）" },
          { value: "indepth", label: "深度（5000字+）" },
        ],
      },
    ],
    systemInstruction:
      "围绕「{{topic}}」制作一篇{{depth}}级别的深度报道。步骤：1)小乐收集热点和素材 2)小深进行深度研究和数据分析 3)小文撰写报道正文 4)小鉴审核质量。每个步骤完成后暂停等待用户确认。",
    promptTemplate:
      "为「{{topic}}」制作一篇深度报道（{{depth}}级别）",
    isFeatured: true,
    steps: [
      step(1, "热点与素材收集", "trending_topics", "热榜聚合", "data_collection", "xiaolei_collect", { maxResults: 10 }),
      step(2, "深度研究与数据分析", "web_search", "全网搜索", "research", "xiaoshen_research", { depth: "indepth" }),
      step(3, "报道撰写", "content_generate", "内容生成", "content_gen", "xiaowen_write", { minLength: 2000 }),
      step(4, "质量审核", "content_review", "内容审核", "quality", "xiaojian_review", { strictness: "high" }),
    ],
  },
];
