 ...
 * executor (`mission-executor.ts`) and the BullMQ workers
...
// `queue/workers/scheduled.ts`. Without this, both call-sites silently
...
export function buildLeaderDecomposePrompt(
  ...
  return `你是穆兰（任务总监），需要将用户的指令分解为多个子任务，并分配给合适的团队成员。
...
- 正文放到 artifact 的 content 字段，并在 body / content 字段同步一份完整稿件，便于入库\n`
  ...
  return `你是穆兰（任务总监），所有子任务已经完成。请汇总所有成果，生成最终的交付物。
