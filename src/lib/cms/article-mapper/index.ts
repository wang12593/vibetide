  ...
  switch (type) {
    ...
    case "5":
      throw new CmsSchemaError(
        "type=5 (视频新闻) 在 P1 不支持；由技术保障部 AIGC 侧自行入库（方案 A，见 spec §1.1）",
      );
 ...
 */
const CMS_SITE_ID = Number(process.env.CMS_DEFAULT_SITE_ID) || 81;
const CMS_APP_ID = Number(process.env.CMS_DEFAULT_APP_ID) || 1768;
const CMS_CATALOG_ID = Number(process.env.CMS_DEFAULT_CATALOG_ID) || 10210;
