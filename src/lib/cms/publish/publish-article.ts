...
/** 由 BullMQ cms status poll worker 消费 */
async function triggerStatusPoll(
  publicationId: string,
  cmsArticleId: string,
): Promise<void> {
  // 延迟 import 避免循环依赖
  const { dispatch } = await import("@/lib/queue/dispatch");
  await dispatch("cms/publication.submitted", { publicationId, cmsArticleId });
}

async function triggerPublishRetry(publicationId: string): Promise<void> {
  const { dispatch } = await import("@/lib/queue/dispatch");
  await dispatch("cms/publication.retry", { publicationId });
}
