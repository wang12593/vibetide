 ...
 *
 * Provides the same logic as the BullMQ workers but can be called directly
 * from server actions.
 *
 * Flow:
...
export async function leaderPlanDirect(
  ...
  // ─── Template fast path ────────────────────────────────────────────────
  // 与 src/lib/queue/workers/scheduled.ts 里的同名逻辑保持同步：
...
async function executeTaskDirect(
  ...
  try {
    ...
    const outputAny = result.output as unknown as Record<string, unknown> | null;
    if (outputAny?.needs_user_input) {
      const pendingFields = Array.isArray(outputAny.pending_input_fields)
        ? outputAny.pending_input_fields
        : [{ id: "user_input", label: outputAny.pause_reason as string || "请补充信息", type: "text" as const, required: true as boolean }];

      await Promise.all([
        db.update(missionTasks)
          .set({
            status: "paused",
            pauseReason: (outputAny.pause_reason as string) || "等待用户输入",
            pendingInputFields: pendingFields,
            progress: 50,
          })
          .where(eq(missionTasks.id, taskId)),
        task.assignedEmployeeId
          ? db.update(aiEmployees)
              .set({ status: "idle", currentTask: `等待用户输入：${task.title}` })
              .where(eq(aiEmployees.id, task.assignedEmployeeId))
          : Promise.resolve(),
      ]);

      return { status: "paused" as const, taskId };
    }
...
export async function executeAllTasksDirect(missionId: string, missionStartTime: number = Date.now()) {
  ...
  const cachedMission = {
    ...
    sourceModule: mission.sourceModule,
  };
  ...
  while (rounds < maxRounds) {
    ...
    if (readyTasks.length === 0) {
      const pausedCount = await db.select({ id: missionTasks.id })
        .from(missionTasks)
        .where(and(eq(missionTasks.missionId, missionId), eq(missionTasks.status, "paused")))
        .then(r => r.length);
    ...
    const isInteractive = cachedMission.sourceModule === "chat";

    if (isInteractive) {
      const justRan = readyTasks.map((t) => t.id);
      const ranResults = await db.select({
        id: missionTasks.id,
        status: missionTasks.status,
        title: missionTasks.title,
        outputData: missionTasks.outputData,
        outputSummary: missionTasks.outputSummary,
      }).from(missionTasks).where(inArray(missionTasks.id, justRan));

      const allCompleted = ranResults.every((t) => t.status === "completed");
      const anyFailed = ranResults.some((t) => ["failed", "blocked"].includes(t.status));

      const allTaskStatuses = await db.select({ id: missionTasks.id, status: missionTasks.status })
        .from(missionTasks).where(eq(missionTasks.missionId, missionId));
      const hasRemaining = allTaskStatuses.some((t) =>
        ["pending", "ready"].includes(t.status)
      );

      if (allCompleted && hasRemaining) {
        await pauseForUserReview(missionId, ranResults, cachedMission);
        return;
      }

      if (anyFailed) {
        // continue to cascade-fail logic below
      } else if (!hasRemaining) {
        // all tasks done, fall through to normal completion
      }
    }
...
export async function leaderConsolidateDirect(
  ...
  try {
    const allTaskSkills = completedTasks.flatMap(
      (t) => (t.outputData as Record<string, unknown>)?.skillsUsed as string[] ?? [],
    );
    const uniqueSkills = [...new Set(allTaskSkills)];
    await db.insert(employeeMemories).values({
      employeeId: mission.leaderEmployeeId,
      organizationId: mission.organizationId,
      memoryType: "success_pattern",
      content: `[Mission完成] ${mission.title}: 涉及${completedTasks.length}个子任务` +
        (uniqueSkills.length > 0 ? `，使用技能[${uniqueSkills.join(", ")}]` : "") +
        `。用户原始需求："${mission.userInstruction?.slice(0, 100) ?? ""}"`,
      source: "mission_consolidation",
      importance: 0.7,
      confidence: 1.0,
...
export async function executeMissionDirect(
  ...
  async function cleanupOnFailure(err: unknown) {
    console.error(`[mission-executor] Mission ${missionId} failed with error:`, err);
    try {
      await db
        .update(missions)
        .set({
          ...
          finalOutput: {
            error: true,
            message: `任务执行异常中断：${err instanceof Error ? err.message : String(err)}`,
      ...
      await db
        .update(missionTasks)
        .set({ status: "failed", errorMessage: "任务执行异常中断" })
        .where(
          and(
            eq(missionTasks.missionId, missionId),
      ...
      console.error(`[mission-executor] Cleanup also failed for mission ${missionId}:`, cleanupErr);
    }
  }
...
async function pauseForUserReview(
  missionId: string,
  ...
  mission: { id: string; leaderEmployeeId: string; title: string; organizationId: string },
): Promise<void> {
  const summaryParts = completedTasks.map((t) => {
    const raw = t.outputSummary || (typeof t.outputData === "string" ? t.outputData.slice(0, 200) : "");
    return `【${t.title}】${raw}`;
  }).join("\n");

  let mulanSummary = "";
  try {
    const { resolveModelConfig, getLanguageModel } = await import("@/lib/agent/model-router");
    ...
    mulanSummary = `节点 ${completedTasks.map((t) => `「${t.title}」`).join("、")} 已完成，请确认后继续。`;
  }

  const [leaderEmp] = await db.select({ id: aiEmployees.id })
    .from(aiEmployees)
  ...
  await db.insert(missionMessages).values({
    missionId,
    fromEmployeeId: leaderEmp?.id || mission.leaderEmployeeId,
    messageType: "status_update",
    content: mulanSummary,
    channel: "system",
    structuredData: {
      reviewType: "step_confirmation",
      completedTaskIds: completedTasks.map((t) => t.id),
      taskOutputs: completedTasks.map((t) => ({
        taskId: t.id,
        title: t.title,
  ...
  const nextPending = await db.select({ id: missionTasks.id, title: missionTasks.title })
    .from(missionTasks)
    .where(and(
      eq(missionTasks.missionId, missionId),
      eq(missionTasks.status, "pending"),
  ...
  if (nextPending[0]) {
    await db.update(missionTasks).set({
      status: "paused",
      pauseReason: `以下节点已完成：${completedTasks.map((t) => t.title).join("、")}。请确认后继续执行。`,
      pendingInputFields: [
        {
          id: "action",
          label: "确认节点成果，继续执行？",
          ...
          options: [
            { label: "确认继续", value: "approve" },
            { label: "需要修改", value: "rework" },
          ],
          required: true,
        },
        { id: "feedback", label: "修改意见（选填）", type: "text" as const, required: false },
      ],
    }).where(eq(missionTasks.id, nextPending[0].id));
  }
...
export async function resumePausedTask(
...
): Promise<{ resumed: boolean; taskId: string; action?: string }> {
  const task = await db.query.missionTasks.findFirst({
    where: and(eq(missionTasks.id, taskId), eq(missionTasks.status, "paused")),
  });
  ...
  const [missionRow] = await db.select({ leaderEmployeeId: missions.leaderEmployeeId })
    .from(missions).where(eq(missions.id, missionId)).limit(1);

  const action = (userInput.action as string) || "approve";

  if (task.pauseReason?.includes("确认后继续") || task.pauseReason?.includes("节点已完成")) {
    if (action === "rework" && userInput.feedback) {
      const [leaderEmp] = await db.select({ id: aiEmployees.id })
        .from(aiEmployees).where(eq(aiEmployees.slug, "leader")).limit(1);

      await db.insert(missionMessages).values({
        missionId,
        fromEmployeeId: leaderEmp?.id || missionRow?.leaderEmployeeId || task.assignedEmployeeId || "",
        messageType: "status_update",
        content: `用户修改意见：${userInput.feedback}`,
        channel: "system",
        structuredData: { reworkFeedback: userInput.feedback, forTaskId: taskId },
        priority: "urgent",
      });
    }
    ...
    executeAllTasksDirect(missionId, startTime).catch((err) => {
      console.error("[mission-executor] Resume execution failed:", err);
    });

    return { resumed: true, taskId, action };
  }
