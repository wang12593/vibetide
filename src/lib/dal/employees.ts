...
export async function getEmployees(opts?: { userId?: string; isAdmin?: boolean }): Promise<AIEmployee[]> {
  const t0 = Date.now();
  const orgId = await getCurrentUserOrg();
  const t1 = Date.now();

  const whereCondition = (opts?.userId && orgId)
  ...
  const t2 = Date.now();
  console.debug(`[dal/employees] auth=${t1 - t0}ms, query=${t2 - t1}ms, rows=${rows.length}`);
  ...
  return uniqueRows.map((emp) => {
    ...
    return {
      ...
      visibility: emp.visibility as "personal" | "org" | undefined,
