 ...
 */
export async function getWorkflowTemplates(opts?: { userId?: string; mode?: "all" | "own" | "org" }) {
  const orgId = await getCurrentUserOrg();

  let whereCondition: SQL | undefined;
  if (orgId) {
    whereCondition = eq(workflowTemplates.organizationId, orgId);
    if (opts?.userId && opts?.mode === "own") {
      whereCondition = and(whereCondition, eq(workflowTemplates.createdBy, opts.userId));
    } else if (opts?.userId && opts?.mode === "org") {
      whereCondition = and(whereCondition, eq(workflowTemplates.isBuiltin, true));
    }
  }
