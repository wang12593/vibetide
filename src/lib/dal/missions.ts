...
export async function getMissions(
  ...
  opts?: { userId?: string; mode?: "all" | "own" | "org" }
): Promise<Mission[]> {
  const whereCond = opts?.userId
    ? await buildVisibilityCondition({
        userId: opts.userId,
...
export async function getMissionById(
  ...
  const tasks: MissionTask[] = taskRows.map((t) => ({
    ...
    pendingInputFields: t.pendingInputFields as MissionTask["pendingInputFields"] ?? null,
...
export async function getMissionTasks(
  ...
  return rows.map((t) => ({
    ...
    errorRecoverable: !!t.errorRecoverable,
    retryCount: t.retryCount,
    pauseReason: (t as Record<string, unknown>).pauseReason as string | null ?? null,
    pendingInputFields: (t as Record<string, unknown>).pendingInputFields as MissionTask["pendingInputFields"] ?? null,
    claimedAt: t.claimedAt?.toISOString() ?? null,
    startedAt: t.startedAt?.toISOString() ?? null,
    completedAt: t.completedAt?.toISOString() ?? null,
    createdAt: t.createdAt.toISOString(),
...
export async function getMissionsWithActiveTasks(
  ...
  const missionWhere = opts?.userId
    ? await buildVisibilityCondition({
        userId: opts.userId,
        orgId: organizationId,
        table: missions,
  ...
  const [missionRows, taskRows, msgRows, empRows, workflowRows] = await Promise.all([
    db.select().from(missions).where(missionWhere).orderBy(desc(missions.createdAt)),
    db.select({
      missionId: missionTasks.missionId,
      status: missionTasks.status,
      title: missionTasks.title,
...
export async function createMissionFromChat(
  orgId: string,
  data: {
    title: string;
    scenario: string;
  ...
  const leaderId = leader?.id ?? data.leaderEmployeeId;
  ...
  const [mission] = await db
    ...
    .values({
      ...
      sourceModule: data.sourceModule ?? "chat",
      sourceEntityId: data.conversationId ?? null,
      sourceEntityType: data.conversationId ? "conversation" : null,
  ...
  if (data.steps && data.steps.length > 0) {
    const slugs = [...new Set(data.steps.map((s) => s.employeeSlug))];
    ...
    const empIdMap = new Map(
      employeeRows.filter(Boolean).map((e) => [e!.slug, e!.id])
    );

    await db.insert(missionTasks).values(
      data.steps.map((step, i) => ({
        missionId: mission.id,
        title: step.taskDescription,
        description: step.taskDescription,
        assignedEmployeeId: empIdMap.get(step.employeeSlug) ?? null,
        assignedRole: step.employeeName,
...
export async function updateMissionStatus(
  missionId: string,
  status: "queued" | "planning" | "executing" | "consolidating" | "completed" | "failed" | "cancelled"
): Promise<void> {
