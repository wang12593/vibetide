...
function mapSkillSummary(s: {
  ...
  return {
    ...
    visibility: (s.visibility as "personal" | "org" | undefined) ?? undefined,
  };
}
...
export async function getSkillsWithBindCount(
  ...
  if (opts?.userId && opts.mode && opts.mode !== "all") {
    rows = rows.filter((r) => {
      if (r.type === "builtin") return true;
      if (opts.mode === "own") return r.createdBy === opts.userId;
      if (opts.mode === "org") return r.visibility === "org" || r.createdBy === opts.userId;
      return true;
    });
  }
