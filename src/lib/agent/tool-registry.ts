...
function scoreNewsItem(item: NewsFeedItem, query: string) {
  ...
  const isCn = url.includes(".cn") || url.includes("china") ||
    ["xinhuanet", "people.com", "cctv", "chinanews", "36kr", "huxiu",
     "tmtpost", "jiemian", "caixin", "yicai", "thepaper", "sina",
     "weibo", "zhihu", "bilibili", "sohu", "163.com", "qq.com",
     "baidu", "toutiao"].some(d => url.includes(d));
  if (isCn) score += 15;
  ...
  return {
    web_search: tool({
      description:
        ...
        "**输出要求：** 在回答末尾必须附加「**来源参考**」段落，列出每条引用的标题和可点击链接，" +
        "格式如：`1. [标题](URL)`。优先引用国内来源（新华网、澎湃、36氪等），" +
        "如引用国外来源必须是国内可直接访问的网站（如 BBC 中文网、Reuters 中文、Wikipedia 中文）。",
    ...
    content_generate: tool({
      ...
      inputSchema: z.object({
        ...
        style: z
          ...
          .describe("写作风格：professional/casual/news/academic/jiexping_style"),
        maxLength: z.number().optional().default(2000).describe("最大字数"),
        personName: z.string().optional().describe("人物姓名（解辛平风格必填）"),
        personTitle: z.string().optional().describe("职务岗位（解辛平风格必填）"),
        personUnit: z.string().optional().describe("所在单位（解辛平风格必填）"),
        achievements: z.string().optional().describe("主要先进事迹（解辛平风格必填）"),
        honors: z.string().optional().describe("所获荣誉（解辛平风格必填）"),
        officialEvaluation: z.string().optional().describe("官方评价（解辛平风格必填）"),
        propagandaFocus: z.string().optional().describe("宣传重点（解辛平风格必填）"),
      }),
      execute: async ({ outline, style, maxLength, personName, personTitle, personUnit, achievements, honors, officialEvaluation, propagandaFocus }) => {
        try {
          const { generateText: gen } = await import("ai");
          const { getLanguageModel, resolveModelConfig } = await import("./model-router");
          const cfg = resolveModelConfig(["info_processing"], { temperature: 0.7, maxTokens: Math.min(maxLength * 2, 8192) });
          const model = getLanguageModel(cfg);

          let prompt = "";
          if (style === "jiexping_style") {
            const personInfo = [
              personName ? `人物姓名：${personName}` : "",
              personTitle ? `职务岗位：${personTitle}` : "",
              personUnit ? `所在单位：${personUnit}` : "",
              achievements ? `主要先进事迹：${achievements}` : "",
              honors ? `所获荣誉：${honors}` : "",
              officialEvaluation ? `官方评价：${officialEvaluation}` : "",
              propagandaFocus ? `宣传重点：${propagandaFocus}` : "",
            ].filter(Boolean).join("\n");

            prompt = `你是一名解放军报重要评论作者，擅长撰写解辛平风格的新闻评论文章。

## 写作要求
- 字数控制在 ${maxLength} 字以内
- 严格按照解辛平风格：政治站位高、思想深刻、语言庄重、用词精准
...
## 语言风格
- 使用"听党指挥、能打胜仗、作风优良"等军语
- 善用排比、对仗句式
- 引用权威文件和领导讲话
- 主旋律突出，正能量充沛

## 人物素材
${personInfo || "（未提供具体素材，请根据大纲创作）"}
...
请直接输出正文内容，不要包含标题和前言。`;
    ...
    fact_check: tool({
      ...
      execute: async ({ text, claims }) => {
        try {
          const { generateText: gen } = await import("ai");
          const { getLanguageModel, resolveModelConfig } = await import("./model-router");
          const cfg = resolveModelConfig(["info_processing"], { temperature: 0.2, maxTokens: 4096 });
          const model = getLanguageModel(cfg);
    ...
    cms_publish: tool({
      description:
        "把一篇稿件真实入库到技术保障部 CMS。**appId / catalogId 已硬编码为 " +
      ...
      execute: async ({
        ...
        //    BullMQ 轮询事件）。feature flag / config 校验都由它内部做，
    ...
    browser_navigate: tool({
      description: "打开指定 URL 页面。返回页面标题、URL 和截图。",
      inputSchema: z.object({
    ...
    browser_screenshot: tool({
      description: "对当前浏览器页面截图。可指定保存路径将截图保存为本地 PNG 文件（如 'C:\\Users\\用户名\\Desktop\\screenshot.png'）。",
      inputSchema: z.object({
        saveToPath: z.string().optional().describe("截图保存到本地的文件路径，留空则仅返回 base64 数据"),
      }),
      execute: async ({ saveToPath }) => browserScreenshot(saveToPath),
    }),
    browser_save_screenshot: tool({
      description: "将当前页面截图保存为本地 PNG 文件。路径示例：'C:\\Users\\用户名\\Desktop\\baidu.png'，支持 ~ 表示用户主目录。",
      inputSchema: z.object({
        filePath: z.string().describe("保存路径，如 'C:\\Users\\wzh\\Desktop\\screenshot.png'"),
      }),
      execute: async ({ filePath }) => browserSaveScreenshot(filePath),
    }),
    browser_fill: tool({
      description: '在指定输入框中填写内容。selector 为 CSS 选择器，如 "#username"、"input[name=email]"。',
      inputSchema: z.object({
        selector: z.string().describe("CSS 选择器"),
        value: z.string().describe("要填写的值"),
      }),
      execute: async ({ selector, value }) => browserFill(selector, value),
    }),
    browser_click: tool({
      description: "点击页面上的元素。selector 为 CSS 选择器。",
      inputSchema: z.object({
        selector: z.string().describe("CSS 选择器"),
      }),
    ...
    browser_select: tool({
      ...
      execute: async ({ selector, value }) => browserSelect(selector, value),
    }),
    browser_submit: tool({
      description: "提交表单。可指定提交按钮的 selector，或留空按 Enter 提交。",
      inputSchema: z.object({
        selector: z.string().optional().describe("提交按钮的 CSS 选择器，留空按 Enter"),
      }),
      execute: async ({ selector }) => browserSubmit(selector),
    }),
    browser_get_text: tool({
      description: "获取页面上指定元素的文本内容。",
      inputSchema: z.object({
        selector: z.string().describe("CSS 选择器"),
      }),
      execute: async ({ selector }) => browserGetText(selector),
    }),
    browser_wait_for: tool({
      description: "等待页面上的指定元素出现。",
      inputSchema: z.object({
        selector: z.string().describe("CSS 选择器"),
        timeout: z.number().optional().default(10000).describe("超时时间(ms)，默认 10000"),
      }),
      execute: async ({ selector, timeout }) => browserWaitFor(selector, timeout),
    }),
    browser_login: tool({
      description: "一站式登录操作：打开页面 → 填写用户名密码 → 点击登录按钮。",
      inputSchema: z.object({
        url: z.string().describe("登录页面 URL"),
        usernameSelector: z.string().describe('用户名输入框 CSS 选择器，如 "#username"'),
        passwordSelector: z.string().describe('密码输入框 CSS 选择器，如 "#password"'),
        username: z.string().describe("用户名"),
        password: z.string().describe("密码"),
        submitSelector: z.string().optional().describe('登录按钮 CSS 选择器，如 "button[type=submit]"'),
      }),
      execute: async ({ url, usernameSelector, passwordSelector, username, password, submitSelector }) =>
        browserLogin(url, usernameSelector, passwordSelector, username, password, submitSelector),
    }),
    browser_close: tool({
 ...
 * 从而保证测试输出跟实际执行输出一致。
 */
const CUSTOM_TOOL_ALIASES = new Set(["agent_browser", "browser_automation"]);

export function isToolRegistered(toolName: string): boolean {
  if (CUSTOM_TOOL_ALIASES.has(toolName)) return true;
...
> {
  const CUSTOM_TO_BUILTIN_EXEC: Record<string, string> = {
    agent_browser: "browser_navigate",
    browser_automation: "browser_navigate",
  };
  const resolvedName = CUSTOM_TO_BUILTIN_EXEC[toolName] ?? toolName;
  const t = ALL_TOOLS[resolvedName];
  if (!t) {
    return {
      ok: false,
      toolName,
...
export function createMissionTools(context: {
  ...
  tools["send_message"] = tool({
    ...
    inputSchema: z.object({
      toEmployeeSlug: z.string().describe("接收者的数字员工slug标识"),
    ...
    execute: async ({ toEmployeeSlug, content }) => {
      ...
      if (!recipient) return { error: `未找到数字员工：${toEmployeeSlug}` };
  ...
  if (context.isLeader) {
    tools["create_task"] = tool({
      ...
      inputSchema: z.object({
        ...
        assignedEmployeeSlug: z.string().describe("分配给哪个数字员工（slug）"),
      ...
      execute: async ({ title, description, expectedOutput, assignedEmployeeSlug, dependencyTitles, priority }) => {
        ...
        if (!emp) return { error: `未找到数字员工：${assignedEmployeeSlug}` };
...
export function toVercelTools(
  ...
  const BROWSER_SKILL_NAMES = new Set(["browser_automation"]);

  for (const t of agentTools) {
    if (ALL_TOOLS[t.name]) {
      result[t.name] = ALL_TOOLS[t.name];
    } else if (BROWSER_SKILL_NAMES.has(t.name)) {
      for (const [toolName, toolDef] of Object.entries(ALL_TOOLS)) {
        if (toolName.startsWith(BROWSER_TOOL_PREFIX) && !BROWSER_SKILL_NAMES.has(toolName)) {
      ...
      const CUSTOM_TO_BUILTIN: Record<string, string[]> = {
        agent_browser: ["browser_navigate", "browser_screenshot", "browser_fill", "browser_click", "browser_select", "browser_submit", "browser_get_text", "browser_wait_for", "browser_login", "browser_close", "browser_save_screenshot"],
        browser_automation: ["browser_navigate", "browser_screenshot", "browser_fill", "browser_click", "browser_select", "browser_submit", "browser_get_text", "browser_wait_for", "browser_login", "browser_close", "browser_save_screenshot"],
      };
      const mappedTools = CUSTOM_TO_BUILTIN[t.name];
      if (mappedTools) {
      ...
      result[t.name] = tool({
        ...
        inputSchema: z.object({
          input: z.string().describe("任务输入内容"),
          instructions: z.string().optional().describe("额外指令"),
        }),
        execute: async ({ input, instructions }) => {
          const { generateText } = await import("ai");
          const { getLanguageModel, resolveModelConfig } = await import("./model-router");
          const modelConfig = resolveModelConfig(["info_processing"], { temperature: 0.7, maxTokens: 4096 });
