...
下面是本步骤对应技能的完整规范（SKILL.md），包含场景 checklist、输出模板与质量要求。你必须严格按其结构产出，不可输出空泛的一句话占位文本（如"XX结果"）。

**SKILL.md 中若提到"应调用场景"、"前置条件"、"输入"或明示工具调用流程，你必须真实调用相应工具，不得跳过工具直接按输出模板想象结果**。参数必须取自用户在【场景输入参数】/【本次场景任务】中的实际输入。工具返回空结果 → 如实说明，不要替换为无关话题。（呼应 system prompt 的"真实性与工具调用"层）
