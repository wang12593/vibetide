...
请以JSON格式返回（不要包含markdown代码块标记）：
{
  ...
  "suggestedSteps": [
    { "key": "步骤key", "label": "步骤名称", "employeeSlug": "数字员工slug" }
  ],
  "reasoning": "规划理由"
}
...
- 每个步骤必须分配一个可用的数字员工slug`;

  try {
    const result = await generateText({
      model: getLanguageModel({ provider: "openai", model: process.env.OPENAI_MODEL || "Qwen3.5-35B-A3B", temperature: 0.3, maxTokens: 2048 }),
