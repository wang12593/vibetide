...
import { getCapabilities } from "./model-router";
...
export function buildSystemPrompt(agent: AssembledAgent): string {
  ...
  const isLeader = agent.slug === "leader";
  const identityCore = isLeader
    ? `你是「${agent.name}」（昵称：${agent.nickname}），职位：${agent.title}。
你是 智能协作空间 内容生产团队的 **调度中枢**。
你的职责是根据用户需求选择最合适的数字员工来执行任务，你不直接执行搜索、写作、分析等技能。
对于简单问题，你可以直接回答；对于复杂任务，你应该分析需求并说明将分派给哪些员工。`
    : `你是「${agent.name}」（昵称：${agent.nickname}），职位：${agent.title}。
你是 智能协作空间 内容生产团队的 数字员工。`;
  const identityLayer = `# 身份
${identityCore}${customInstructions ? `\n\n## 自定义工作指令\n${customInstructions}` : ""}`;
...
5. **参数来源**：调用工具所需的 query / topic / keyword 等参数，必须来自【场景输入参数】或【本次场景任务】中的用户原始输入；不得用步骤名、技能描述里的字面量（如"全网搜索"）或数字输入字段（如 count=2）来当搜索词。
6. **违反后果**：伪造结果将被下游质检判定为无效产出，相关任务会被标记失败。`);
  ...
  if (agent.skillContents && Object.keys(agent.skillContents).length > 0) {
    ...
    if (guides.length > 0) {
      ...
      const guideLimit = caps.promptStyle === "concise" ? 1000 : 3000;
      let combined = guides.join("\n\n");
      if (combined.length > guideLimit) {
        combined = combined.slice(0, guideLimit) + "\n\n(更多技能指南已省略)";
      }
...
- 主动提出风险点和改进建议`);
  ...
  if (agent.memories && agent.memories.length > 0) {
    const caps = getCapabilities();
    const limit = caps.memoryLimit;
    const memoryLines = agent.memories.slice(0, limit).map(
...
${memoryLines.join("\n")}`);
  ...
  // Layer 6.5: User preferences (leader agent only)
  if (agent.userProfile) {
    const profile = agent.userProfile;
    const topIntents = profile.frequentIntents
      .sort((a, b) => b.count - a.count)
      .slice(0, 3)
      .map((f) => `${f.intentType}(${f.count}次)`)
      .join("、");

    const assignments = profile.preferredAssignments
      .filter((a) => a.userConfirmed >= 2)
      .map((a) => `${a.intentType} → ${a.steps.map((s) => s.employeeSlug).join("→")}`)
      .join("；");

    if (topIntents || assignments || profile.communicationPreference) {
      const parts: string[] = [];
      if (topIntents) parts.push(`- 高频需求：${topIntents}`);
      if (assignments) parts.push(`- 偏好分配方案：${assignments}`);
      if (profile.communicationPreference) parts.push(`- 沟通风格：${profile.communicationPreference}`);
      layers.push(`# 用户偏好\n根据历史交互，这位用户有以下偏好，请据此调整回答风格和任务分配策略：\n${parts.join("\n")}`);
    }
  }
...
export function buildLeaderDecomposePrompt(
  ...
  return `你是穆兰（任务总监）。用户下达了一个${scenario}场景的任务，你需要：
...
- assignedEmployeeSlug: 负责数字员工的slug
