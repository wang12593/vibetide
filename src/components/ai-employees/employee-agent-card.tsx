...
import { ArrowUpRight, Plus, UserCog, Loader2, Users } from "lucide-react";
...
export function EmployeeAgentCard({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <VisibilityBadge visibility={employee.visibility} size="xs" />
          </div>
      ...
      <div className="mt-2 pt-2 border-t border-white/40 dark:border-white/5 flex justify-center items-center gap-2">
        <button
        ...
        {employee.visibility === "personal" && (
          <EmployeeUpgradeButton
...
function EmployeeUpgradeButton({
  employeeDbId,
  onDone,
}: {
  employeeDbId: string;
  ...
  async function handleUpgrade() {
    setLoading(true);
    try {
      await upgradeContentVisibility("employee", employeeDbId);
      toast.success("已升级为组织共享");
      onDone();
    } catch (err) {
  ...
  return (
    <button
      className="text-[11px] text-gray-400 hover:text-blue-600 dark:text-gray-500 dark:hover:text-blue-400 transition-colors border-0 bg-transparent cursor-pointer flex items-center gap-0.5"
      onClick={(e) => {
        e.stopPropagation();
        handleUpgrade();
      }}
      disabled={loading}
    >
      {loading ? <Loader2 size={10} className="animate-spin" /> : <Users size={10} />}
      {loading ? "升级中..." : "升级为组织共享"}
    </button>
  );
}
