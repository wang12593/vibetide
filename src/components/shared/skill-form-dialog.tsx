...
const categoryOptions: { value: SkillCategory; label: string }[] = [
  { value: "info_perception", label: "信息感知" },
  { value: "info_processing", label: "信息加工" },
  { value: "system_interop", label: "系统互操作" },
  { value: "automation", label: "自动化工具" },
  { value: "communication", label: "通讯协作" },
  { value: "multimodal", label: "多模态处理" },
...
export function SkillFormDialog({
  ...
  const [category, setCategory] = useState<SkillCategory>(
    initialData?.category ?? "info_perception"
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      setCategory(initialData?.category ?? "info_perception");
