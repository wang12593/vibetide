...
import { FolderOpen, Radio, FileText, Bell, User } from "lucide-react";
...
interface StepConfigPanelProps {
  ...
  employees?: Array<{ slug: string; name: string }>;
  skills?: Array<{ slug: string; name: string; category: string; type?: string }>;
}
...
export function StepConfigPanel({
  ...
  const allSkills: SkillOption[] = (() => {
    const builtinSlugs = new Set(SKILL_OPTIONS.map((s) => s.slug));
    const custom = (dynamicSkills ?? [])
      .filter((s) => !builtinSlugs.has(s.slug))
      .map((s) => ({ slug: s.slug, name: s.name, category: s.category }));
    return [...SKILL_OPTIONS, ...custom];
  })();
  ...
  useEffect(() => {
    if (step) {
      ...
      setEmployeeSlug(step.config?.employeeSlug ?? "");
    }
  }, [step]);
  ...
  function handleSave() {
    ...
    const selectedSkill = allSkills.find((s) => s.slug === skillSlug);
            ...
            <>
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select value={employeeSlug} onValueChange={setEmployeeSlug}>
                  ...
                  <SelectContent>
                    <SelectItem value="">自动分配（按技能匹配）</SelectItem>
                    {(employees?.length ? employees : Object.values(EMPLOYEE_META).map(meta => ({ slug: meta.id as string, name: meta.name }))).map((emp, index) => {
                      const colors = [
