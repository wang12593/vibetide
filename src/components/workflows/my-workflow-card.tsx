  ...
  return (
    <GlassCard hover className="group flex flex-col">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className={`shrink-0 text-[11px] px-2 py-0.5 rounded-full ${
          workflow.isEnabled
            ...
            : "bg-gray-100 dark:bg-white/[0.08] text-gray-400 dark:text-white/30"
        }`}>
          {workflow.isEnabled ? "已启用" : "已禁用"}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 mt-auto opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        {onToggleEnabled && (
          <button
            onClick={() => onToggleEnabled(workflow.id)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm border-0 cursor-pointer transition-all ${
              workflow.isEnabled
                ? "bg-red-50/80 dark:bg-red-500/[0.08] text-red-500 dark:text-red-400 hover:bg-red-100/90 dark:hover:bg-red-500/[0.15] hover:text-red-600 dark:hover:text-red-300"
                : "bg-green-50/80 dark:bg-green-500/[0.08] text-green-600 dark:text-green-400 hover:bg-green-100/90 dark:hover:bg-green-500/[0.15] hover:text-green-700 dark:hover:text-green-300"
            }`}
          >
            <Zap className="w-3.5 h-3.5" />
            {workflow.isEnabled ? "禁用" : "启用"}
          </button>
        )}
        <button
