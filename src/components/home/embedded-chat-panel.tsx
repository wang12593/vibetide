...
export function EmbeddedChatPanel({
  ...
  const lockedUserMessageRef = useRef<string>("");
  useEffect(() => {
    if (chat.pendingMessage && !lockedUserMessageRef.current) {
      lockedUserMessageRef.current = chat.pendingMessage;
    }
  }, [chat.pendingMessage]);
          ...
          {chat.messages.map((msg, i) => {
                    ...
                    <div className="mt-3 space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <a
                          ...
                          download
                          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/60 dark:bg-white/5 hover:bg-white dark:hover:bg-white/10 transition-colors cursor-pointer group flex-1"
                        >
            ...
            return (
              <React.Fragment key={i}>
                ...
                {showIntentHere && chat.multiTurnState.active && chat.pendingIntent?.needsClarification && chat.pendingIntent.clarificationQuestions?.[0] && !chat.intentLoading && (
                  <div className="my-3">
                    <MultiTurnClarifyBubble
                      question={chat.pendingIntent.clarificationQuestions[0]}
                      round={chat.multiTurnState.round}
                      maxRounds={chat.MAX_CLARIFY_ROUNDS}
                      onAnswer={chat.handleMultiTurnClarify}
                      onSkip={chat.skipMultiTurnClarify}
                    />
                  </div>
                )}
                {showIntentHere && chat.pendingIntent && !chat.intentLoading && !chat.multiTurnState.active && (() => {
                    ...
                    if (pi.steps.length === 1) {
                      ...
                      return (
                        <div className="flex justify-center gap-3">
                          ...
                          <button
                            onClick={() => {
                              const userMsg = pi.originalMessage || lockedUserMessageRef.current || chat.pendingMessage || step.taskDescription || "";
                              chat.cancelIntent();
                              setTimeout(() => {
                                chat.sendMessage(userMsg, undefined, {
                                  skipIntent: true,
                            ...
                            className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white/30 dark:bg-gray-800/20 hover:bg-white/50 dark:hover:bg-gray-800/40 transition-all border border-gray-200/20 dark:border-gray-700/20"
                          >
                            <span className="text-xs text-gray-400">🏠 就在这里完成</span>
                          </button>
                    ...
                    if (pi.confidence < 0.8 && !chat.loading && !chat.isStreaming && isLastUserMsg) {
                      return (
                        <IntentConfirmCard
                          intent={pi}
                          onConfirm={(editedIntent) => {
                            chat.executeIntent(chat.pendingMessage, editedIntent, true);
                          }}
                          onCancel={chat.cancelIntent}
                        />
                      );
