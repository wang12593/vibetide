        ...
        <div className="max-w-4xl mx-auto px-6 md:px-10 py-8">
          ...
          <SectionWrapper id="ch1-layers">
            ...
            <div className="space-y-4 text-muted-foreground leading-relaxed">
              <p>
                ...
                ：提供数据库存储（Supabase PostgreSQL）、大语言模型调用（DeepSeek/OpenAI 兼容接口）、向量嵌入计算（Jina Embeddings）、后台任务调度（BullMQ）等基础能力。组件可替换，上层无感知。
              </p>
              <p>
          ...
          <SectionWrapper id="ch2-skills">
            ...
            <ul className="list-disc pl-5 space-y-1 text-sm text-muted-foreground mb-4">
              <li><strong className="text-foreground">能力复用</strong>——同一种写稿能力被硬编码在每个数字员工里，修改一处需改多处</li>
              <li><strong className="text-foreground">能力量化</strong>——无法衡量数字员工对某项任务的熟练程度，分配只能靠静态规则</li>
              <li><strong className="text-foreground">能力扩展</strong>——新增一个能力需要改动数字员工定义，而非简单挂载</li>
            </ul>
            <div className="my-4 rounded-xl bg-primary/5 border border-primary/10 px-5 py-4 text-sm leading-relaxed">
              🔑 <strong className="text-foreground">技能 = 能力的原子单元</strong>。每个技能包含名称、执行指南（guideline）、工具绑定、权限等级。通过多对多绑定挂载到数字员工，可量化熟练度（0-100 分，三档行为指导）。
            </div>
          ...
          <SectionWrapper id="ch3-pipeline">
            ...
            <p className="text-muted-foreground leading-relaxed mb-4">
              支持三种入库方式：手动粘贴、文件上传（.md/.txt）、URL 抓取（Jina Reader）。入库后经过智能分块（500-800 字/块，50 字重叠）、Jina Embeddings v3 向量化（1024 维），异步由 BullMQ 调度完成。
            </p>
            <KnowledgePipeline />
          </SectionWrapper>

          <SectionWrapper id="ch3-binding">
            <SectionHeading>数字员工 × 知识库绑定机制</SectionHeading>
            <p className="text-muted-foreground leading-relaxed mb-4">
              知识库与数字员工通过<strong className="text-foreground">多对多绑定</strong>关联。一个数字员工可绑定多个知识库（如小文同时绑定「写作规范库」和「行业资讯库」），一个知识库也可被多名数字员工共享。
            </p>
            <p className="text-muted-foreground leading-relaxed mb-4">
              执行时，Agent Assembly 自动检查数字员工已绑定且 <code className="text-primary text-sm">vectorization_status = done</code> 的知识库，注入 <code className="text-primary text-sm">kb_search</code> 工具。数字员工在需要时自主调用该工具进行语义检索。
            </p>
            <div className="my-4 rounded-xl bg-primary/5 border border-primary/10 px-5 py-4 text-sm leading-relaxed">
          ...
          <SectionWrapper id="ch4">
            <ChapterHeader
              number="CHAPTER 04"
              title="认知进化闭环"
              subtitle="系统最核心的设计——让 数字员工不是静态的 Prompt 机器人，而是能持续进化的智能体。"
            />
          </SectionWrapper>
          ...
          <SectionWrapper id="ch4-skill">
            ...
            <p className="text-muted-foreground leading-relaxed mb-4 text-sm">
              未来规划：技能发现机制——当 数字员工在执行中展现出未注册的能力模式时，系统自动建议创建新技能。
            </p>
            <div className="my-4 rounded-xl bg-accent/5 border border-accent/10 px-5 py-4 text-sm">
          ...
          <SectionWrapper id="ch4-exec">
            ...
            <p className="text-muted-foreground leading-relaxed mb-4">
              每次执行前，<strong className="text-foreground">Agent Assembly 引擎</strong>动态组装执行上下文。流程：加载数字员工档案 → 提取技能集合与熟练度 → 检索已绑定知识库 → 拉取近 10 条相关记忆 → 计算工具权限 → 构建 7 层系统提示词。
            </p>
            <SevenLayerPrompt />
            <div className="my-4 rounded-xl bg-accent/5 border border-accent/10 px-5 py-4 text-sm">
          ...
          <SectionWrapper id="ch4-flywheel">
            ...
            <p className="text-muted-foreground leading-relaxed mb-4">
              <strong className="text-foreground">第一圈</strong>：数字员工根据基础 Prompt 执行，输出质量中等。
              <strong className="text-foreground">第二圈</strong>：记忆注入后，数字员工了解用户偏好，输出更贴合需求。
              <strong className="text-foreground">第三圈</strong>：聚合学习发现成功模式，技能熟练度提升，Prompt 策略自动升档。
              <strong className="text-foreground">第 N 圈</strong>：知识库持续更新、记忆不断积累、数字员工从「新手」成长为「专家」。
            </p>
            <div className="my-4 rounded-xl bg-primary/5 border border-primary/10 px-5 py-4 text-sm leading-relaxed">
              🔑 <strong className="text-foreground">与传统 AI 应用的根本区别</strong>：大多数 AI 应用是无状态的——每次对话从零开始。我们通过<strong className="text-foreground">知识库 + 记忆 + 学习引擎</strong>三位一体，让每个 数字员工具备有状态的持续进化能力。系统会越用越好。
            </div>
          </SectionWrapper>
          ...
          <SectionWrapper id="ch5">
            ...
            />
            <p className="text-muted-foreground leading-relaxed mb-4">
              核心技术选型与实现状态一览。前端采用 Next.js 16 + React 19 + Tailwind v4，后端基于 Supabase PostgreSQL + Drizzle ORM，AI 层使用 AI SDK v6 对接 DeepSeek，后台任务由 BullMQ 驱动。
            </p>
            <TechStackCards />
          </SectionWrapper>
