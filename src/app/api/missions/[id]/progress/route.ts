...
export async function GET(
  ...
  const ownerCheck = await db.select({ organizationId: missions.organizationId })
    .from(missions).where(eq(missions.id, missionId)).limit(1);
  if (!ownerCheck[0] || ownerCheck[0].organizationId !== authResult.organizationId) {
          ...
          for (const t of tasks) {
            ...
            if (prev !== t.status) {
              send("task-update", {
                ...
                pendingInputFields: t.pendingInputFields,
              });

              if (t.status === "paused" && t.pauseReason && t.pauseReason.includes("确认后继续")) {
                send("review-required", {
                  taskId: t.id,
                  title: t.title,
                  pauseReason: t.pauseReason,
                  pendingInputFields: t.pendingInputFields,
