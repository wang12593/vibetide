 ...
 * 逻辑），最后 SSE 一次性返回 complete。Cron 路径仍走 BullMQ（带重试 + 异步）。
 *
 * Enrichment 事件 (`hot-topics/enrich-requested`) 仍 fire-and-forget 派发——
 * 如果 dispatch 失败只是 enrich 不跑，hot_topics 行已经存在，UI 能看到新数据。
 */
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // 6. fire-and-forget enrichment（通过 BullMQ dispatch 触发；失败则跳过，不影响列表展示）
        if (enrichTopicIds.length > 0) {
          dispatch("hot-topics/enrich-requested", {
            organizationId: orgId,
            topicIds: enrichTopicIds,
          }).catch((err) => {
