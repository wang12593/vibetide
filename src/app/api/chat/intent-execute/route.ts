...
import { db } from "@/db";
import { aiEmployees, userProfiles, intentLogs, workflowTemplates, missionTasks } from "@/db/schema";
...
import { startMissionFromChat } from "@/app/actions/missions";
import { updateMissionStatus } from "@/lib/dal/missions";
...
export async function POST(req: Request) {
  try {
    ...
    const { message, intent, conversationHistory, userEdited, resumeFromStep, priorStepOutput: resumePriorOutput, reviewFeedback, conversationId, clarifiedParameters } = body as {
      message: string;
      intent: IntentResult;
    ...
    console.log("[intent-execute] resumeFromStep:", resumeFromStep, "reviewFeedback:", reviewFeedback ?? "(none)", "message:", message?.slice(0, 50));
    ...
    // When a workflow is matched, override LLM-generated steps with template steps
    if (intent.workflowId && orgId) {
      const wfTemplate = await db.query.workflowTemplates.findFirst({
        where: and(
          eq(workflowTemplates.id, intent.workflowId),
          eq(workflowTemplates.organizationId, orgId)
        ),
      });
      if (wfTemplate?.steps) {
        const templateSteps = wfTemplate.steps as Array<{
          id?: string;
          name?: string;
          order?: number;
          dependsOn?: string[];
        ...
        const defaultTeam = (wfTemplate.defaultTeam as string[]) || [];
        const EMPLOYEE_SLUG_NAMES: Record<string, string> = {
          leader: "穆兰", xiaolei: "小雷", xiaoce: "小策", xiaozi: "小子",
          xiaowen: "小文", xiaojian: "小剪", xiaoshen: "小深", xiaofa: "小发", xiaoshu: "小数",
        };
        if (templateSteps.length > 0) {
          intent.steps = templateSteps.map((ts, idx) => {
            const slug = (defaultTeam[idx] || defaultTeam[0] || intent.steps[0]?.employeeSlug || "leader") as EmployeeId;
            return {
              employeeSlug: slug,
              employeeName: EMPLOYEE_SLUG_NAMES[slug] || slug,
              taskDescription: ts.name || ts.config?.skillName || "执行任务",
              skills: ts.config?.skillSlug ? [ts.config.skillSlug] : ["web_search"],
            };
          });
          console.log("[intent-execute] Overrode steps from workflow template:", intent.steps.length, "steps",
            intent.steps.map(s => `${s.employeeName}(${s.employeeSlug}): ${s.taskDescription}`).join(", "));
        }
      }
    }
    ...
    // Create Mission from Chat (await to ensure missionId is available)
    const chatLeader = employeeMap.get("leader") ?? employeeMap.get(intent.steps[0].employeeSlug);
    let missionResult: { id: string } | null = null;
    if (chatLeader) {
      const teamMemberIds = [...new Set(
        intent.steps.map((s) => employeeMap.get(s.employeeSlug)?.id).filter(Boolean) as string[]
      )];
      try {
        missionResult = await startMissionFromChat({
          orgId,
          title: intent.summary || message.slice(0, 60),
          scenario: intent.intentType,
          userInstruction: message,
          leaderEmployeeId: chatLeader.id,
          conversationId,
          clarifiedParameters,
          steps: intent.steps,
        ...
        if (missionResult) {
          await updateMissionStatus(missionResult.id, "executing");
        }
      } catch (err) {
        console.error("[intent-execute] startMissionFromChat failed:", err);
      }
    }
    ...
    const updateMissionTaskStatus = (stepIndex: number, status: "completed" | "failed" | "in_progress", output?: string) => {
      const emp = employeeMap.get(intent.steps[stepIndex]?.employeeSlug);
      if (!emp || !missionResult?.id) return;
      db.update(missionTasks)
        .set({
          status,
          outputSummary: output?.slice(0, 500) ?? null,
          ...(status === "completed" ? { completedAt: new Date() } : {}),
        })
        .where(
          and(
            eq(missionTasks.missionId, missionResult.id),
            eq(missionTasks.assignedEmployeeId, emp.id),
            eq(missionTasks.priority, stepIndex),
          )
        )
        .catch((err) =>
          console.error("[intent-execute] Failed to update mission task:", err)
        );
    };
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        let priorStepOutput = resumePriorOutput || "";
        let streamHadError = false;
        let isLastStepReached = false;

        try {
          const startStep = resumeFromStep ?? 0;

          for (let i = startStep; i < intent.steps.length; i++) {
            ...
            //
            // 关键坑：xiaolei 等数字员工的技能里有 trend_monitor / heat_scoring /
...
*本步骤由 server 端直接执行，未经 LLM 改写。*`;
                ...
                updateMissionTaskStatus(i, "completed", stepText);
                ...
                const isLast1 = i === intent.steps.length - 1;
                isLastStepReached = isLast1;
                send("step-review", {
                  stepIndex: i,
                  totalSteps: intent.steps.length,
                  employeeSlug: step.employeeSlug,
                  ...
                  outputPreview: stepText,
                  nextStepDescription: isLast1 ? "工作流已完成" : intent.steps[i + 1].taskDescription,
                  nextStepEmployee: isLast1 ? "" : intent.steps[i + 1].employeeName,
                  intent,
                  priorStepOutput: stepText,
                  isLastStep: isLast1,
                });
...
*本步骤由 server 端直接从 Tavily 实时返回，未经 LLM 改写，保证来源、标题、日期、URL 100% 原样。下游稿件撰写步骤请严格基于以上真实数据，不得引入训练数据里的旧内容。*`;
              ...
              const isLast2 = i === intent.steps.length - 1;
              isLastStepReached = isLast2;
              send("step-review", {
                stepIndex: i,
                totalSteps: intent.steps.length,
                employeeSlug: step.employeeSlug,
                ...
                taskTitle: step.taskDescription,
                outputPreview: stepText,
                nextStepDescription: isLast2 ? "工作流已完成" : intent.steps[i + 1].taskDescription,
                nextStepEmployee: isLast2 ? "" : intent.steps[i + 1].employeeName,
                intent,
                priorStepOutput: stepText,
                isLastStep: isLast2,
              });
            ...
            let execSystemPrompt: string = agent.systemPrompt;
            let execMessages = messages;
            if (execSystemPrompt.length > MAX_INPUT_CHARS) {
              execSystemPrompt = execSystemPrompt.slice(0, MAX_INPUT_CHARS) + "\n\n(系统提示已截断)";
            }
            while (execSystemPrompt.length + JSON.stringify(execMessages).length > MAX_INPUT_CHARS && execMessages.length > 1) {
              execMessages = execMessages.slice(1);
            }
            ...
            const isLastStep = i === intent.steps.length - 1;
            isLastStepReached = isLastStep;
            console.log("[intent-execute] Sending step-review for step", i, "of", intent.steps.length, "employee:", step.employeeName, "isLast:", isLastStep);
            send("step-review", {
              stepIndex: i,
              totalSteps: intent.steps.length,
              employeeSlug: step.employeeSlug,
              ...
              taskTitle: step.taskDescription,
              outputPreview: stepText,
              nextStepDescription: isLastStep ? "工作流已完成" : intent.steps[i + 1].taskDescription,
              nextStepEmployee: isLastStep ? "" : intent.steps[i + 1].employeeName,
              intent,
              priorStepOutput: stepText,
              isLastStep,
            });
          ...
          if (missionResult?.id) {
            if (streamHadError) {
              updateMissionStatus(missionResult.id, "failed").catch((e: unknown) =>
                console.error("[intent-execute] Failed to update mission status to failed:", e)
              );
            } else if (isLastStepReached) {
              updateMissionStatus(missionResult.id, "completed").catch((e: unknown) =>
                console.error("[intent-execute] Failed to update mission status to completed:", e)
              );
            }
          }
