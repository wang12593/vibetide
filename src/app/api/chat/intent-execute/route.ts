...
*本步骤由 server 端直接执行，未经 LLM 改写。*`;
                ...
                send("step-review", {
                  ...
                  nextStepDescription: isLast1 ? "场景已完成" : intent.steps[i + 1].taskDescription,
...
*本步骤由 server 端直接从 Tavily 实时返回，未经 LLM 改写，保证来源、标题、日期、URL 100% 原样。下游稿件撰写步骤请严格基于以上真实数据，不得引入训练数据里的旧内容。*`;
              ...
              send("step-review", {
                ...
                taskTitle: step.taskDescription,
                outputPreview: stepText,
                nextStepDescription: isLast2 ? "场景已完成" : intent.steps[i + 1].taskDescription,
            ...
            send("step-review", {
              ...
              taskTitle: step.taskDescription,
              outputPreview: stepText,
              nextStepDescription: isLastStep ? "场景已完成" : intent.steps[i + 1].taskDescription,
