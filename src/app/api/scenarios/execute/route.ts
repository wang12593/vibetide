...
export async function POST(req: Request) {
  try {
    ...
    let scenarioSystem = `${agent.systemPrompt}\n\n# 当前场景任务\n${resolvedInstruction}`;
    let scenarioMessages = messages;
    if (scenarioSystem.length > MAX_INPUT_CHARS) {
      scenarioSystem = scenarioSystem.slice(0, MAX_INPUT_CHARS) + "\n\n(系统提示已截断)";
    }
    while (scenarioSystem.length + JSON.stringify(scenarioMessages).length > MAX_INPUT_CHARS && scenarioMessages.length > 1) {
      scenarioMessages = scenarioMessages.slice(1);
    }
