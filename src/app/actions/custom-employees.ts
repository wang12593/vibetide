...
export async function createCustomEmployee(input: {
  ...
  if (trimmedName.length > 50) throw new Error("数字员工名称过长");
...
export async function updateCustomEmployee(
  ...
  if (input.name !== undefined) {
    ...
    if (!trimmed) throw new Error("数字员工名称不能为空");
    if (trimmed.length > 50) throw new Error("数字员工名称过长");
  ...
  if (input.instructions !== undefined || input.visibility !== undefined) {
    ...
    const merged = {
      ...
      ...(input.visibility !== undefined ? { visibility: input.visibility === "private" ? "personal" : input.visibility } : {}),
    };
