...
import { assertContentOwnership } from "@/lib/dal/visibility-filter";
import { createMissionFromChat } from "@/lib/dal/missions";
 ...
 *
 * Uses direct execution by default.
 ...
 * Does NOT require user auth — can be called from BullMQ workers.
 */
...
export async function deleteMission(missionId: string) {
  ...
  assertContentOwnership(mission, user.id);
...
/**
 * 重置卡在 working 状态超过 10 分钟的数字员工，并将其 in_progress 任务标记为失败。
 */
 ...
 * 执行顺序：先清理数字员工 → 再清理 mission。
 */
export async function cleanupStuckMissions() {
  const orgId = await getCurrentUserOrg();
  if (!orgId) return;

  // Step 1: 清理卡住的数字员工和子任务
...
export async function sendMissionMessage(
  ...
  const mission = await db.query.missions.findFirst({
    ...
    columns: { id: true, leaderEmployeeId: true },
  });
  if (!mission) throw new Error("任务不存在或无权操作");
...
export async function startMissionFromChat(data: {
  orgId: string;
  title: string;
  scenario: string;
  userInstruction: string;
  ...
  try {
    await requireAuth();
    const result = await createMissionFromChat(data.orgId, {
      title: data.title,
      scenario: data.scenario,
      userInstruction: data.userInstruction,
      leaderEmployeeId: data.leaderEmployeeId,
      conversationId: data.conversationId,
      clarifiedParameters: data.clarifiedParameters,
      sourceModule: "chat",
      steps: data.steps,
    ...
    console.error("[startMissionFromChat] Failed:", err);
  }
}

export async function addMissionMembers(
  missionId: string,
  employeeIds: string[],
) {
  await requireAuth();
  ...
  const mission = await db.query.missions.findFirst({
    ...
    columns: { id: true, teamMembers: true, leaderEmployeeId: true },
  });
  if (!mission) throw new Error("任务不存在或无权操作");

  const currentMembers = (mission.teamMembers as string[]) ?? [];
  const newMembers = [...new Set([...currentMembers, ...employeeIds])];
  ...
  await db.insert(missionMessages).values({
    ...
    content: `团队成员已更新，当前共 ${newMembers.length} 人`,
    channel: "system",
    priority: "normal",
  });
