...
import { createMissionGroupChat } from "@/app/actions/group-chat";
 ...
 *   7. Dispatch `mission/created` BullMQ job so the existing executor picks it up.
 *
...
export async function startMissionFromTemplate(
  ...
  // reported by the user as "每次都是排在第一个要做事情的数字员工". That's
  ...
  if (defaultTeamSlugs.length >= 2) {
    const authUser = await createClient().then((sb) => sb.auth.getUser());
    const userId = authUser.data.user?.id;
    if (userId) {
      createMissionGroupChat({
        missionId: created.id,
        missionTitle: template.name,
        employeeSlugs: defaultTeamSlugs,
        leaderEmployeeSlug: "leader",
        userId,
        orgId,
      }).catch((err) =>
        console.error("[workflow-launch] auto group chat failed:", err)
      );
    }
  }

  // Fire-and-forget inline execution（对齐 src/app/actions/missions.ts:170 的 startMission 老路径）：
  // dev 环境下也能推进 mission；production 同样走 executeMissionDirect。
