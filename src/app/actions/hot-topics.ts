 ...
 * Called directly from the UI "刷新热点" button.
 */
...
// enrichment BullMQ pipeline）；如果该字段为空会 fallback 到 CATEGORY_ANGLES
// 模板套话，与原文无关，用户体验差。
//
...
- oneLineSummary 是新闻 5W1H 的精炼，不是评论`;
  ...
  const config = resolveModelConfig(["info_processing"], { temperature: 0.3, maxTokens: 1024 });
...
- summary 不要出现"本文将"等套话，直接说重点`;
  ...
  const config = resolveModelConfig(["info_processing"], { temperature: 0.6, maxTokens: 4096 });
  const model = getLanguageModel(config);
...
export async function startTopicMissionMulti(
  ...
  if (!leaderId || !collectorId || !writerId || !auditorId) {
    throw new Error("缺少必要数字员工（xiaolei/xiaozi/xiaowen/xiaoshen），请先 seed 数字员工");
  }
...
//   - BullMQ scheduled worker `dailyHotBriefingCron`（trigger='scheduled', 每 org 调一次）
...
- 不要包裹 <html> / <body> / <head> 标签，只输出内容片段`;
  ...
  const config = resolveModelConfig(["info_processing"], {
    temperature: 0.4,
    maxTokens: 4096,
  });
  const model = getLanguageModel(config);
