...
export async function importSkillMd(data: {
  ...
  const [newSkill] = await db
    ...
    .values({
      organizationId: orgId,
      slug: generateSlug(meta.displayName || name),
...
// ---------------------------------------------------------------------------
// Internal: Record Skill Usage (no auth required, for BullMQ background tasks)
// ---------------------------------------------------------------------------

/**
 * Internal function for recording skill usage without user session.
 * Used by BullMQ background tasks (execute-mission-task).
 */
