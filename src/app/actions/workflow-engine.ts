...
export async function createWorkflowFromTemplate(templateId: string) {
  ...
  if (virtual) {
    ...
    const [newWorkflow] = await db
      ...
      .values({
        ...
        isEnabled: false,
        createdBy: user.id,
        inputFields: (virtual as any).inputFields ?? [],
        promptTemplate: (virtual as any).promptTemplate ?? null,
        defaultTeam: (virtual as any).defaultTeam ?? [],
      })
      .returning();
  ...
  const [newWorkflow] = await db
    ...
    .values({
      ...
      isEnabled: false,
      createdBy: user.id,
      inputFields: (template as any).inputFields ?? [],
      promptTemplate: (template as any).promptTemplate ?? null,
      defaultTeam: (template as any).defaultTeam ?? [],
    })
    .returning();
