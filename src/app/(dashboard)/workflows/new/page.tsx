...
export default async function NewWorkflowPage() {
  ...
  const [skills, employees] = await Promise.all([
    listSkillsForWorkflowPicker(user ? { userId: user.id, isAdmin: admin } : undefined).catch((err) => { console.error("[workflows/new] load skills failed:", err); return []; }),
    getEmployees(user ? { userId: user.id, isAdmin: admin } : undefined).catch((err) => { console.error("[workflows/new] load employees failed:", err); return []; }),
  ]);
