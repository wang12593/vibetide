...
import { reviewTaskApproval, batchReviewTaskApprovals } from "@/app/actions/approvals";
...
export function ApprovalsClient({
  ...
  function handleApprove(item: PendingApproval) {
    startTransition(async () => {
      await reviewTaskApproval(item.stepId, "completed", feedbackMap[item.stepId] || undefined);
      setFeedbackMap((prev) => {
        const next = { ...prev };
        delete next[item.stepId];
        return next;
      });
      router.refresh();
    });
  ...
  function handleReject(item: PendingApproval) {
    startTransition(async () => {
      await reviewTaskApproval(item.stepId, "failed", feedbackMap[item.stepId] || undefined);
      setFeedbackMap((prev) => {
        const next = { ...prev };
        delete next[item.stepId];
        return next;
  ...
  function handleBatchApprove() {
    startTransition(async () => {
      await batchReviewTaskApprovals(Array.from(selectedIds), "completed");
