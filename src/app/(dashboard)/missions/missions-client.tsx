...
const SOURCE_MODULE_LABEL: Record<string, { label: string; cls: string }> = {
  ...
  chat:         { label: "来自对话", cls: "bg-sky-100 dark:bg-sky-900/20 text-sky-700 dark:text-sky-400" },
};
            ...
            <AlertDialogDescription>
              {activeConflict ? (
                <>
                  ...
                  仍要新建吗？同时跑多条同模板的任务可能浪费 token 与数字员工算力。
                </>
              ) : null}
            </AlertDialogDescription>
          </AlertDialogHeader>
...
function UpgradeButton({
  ...
  async function handleUpgrade() {
    setLoading(true);
    try {
      await upgradeContentVisibility(contentType, id);
  ...
  return (
    <Button
      variant="ghost"
      size="sm"
      className="gap-1 text-xs text-gray-400 hover:text-blue-600 dark:text-gray-500 dark:hover:text-blue-400 h-7"
              ...
              <div className="relative grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr_auto] gap-6 p-4 rounded-xl bg-blue-50/50 dark:bg-blue-950/20 overflow-hidden">
                ...
                <div className="flex items-end justify-end gap-2">
                  {m.visibility === "personal" && (
                    <UpgradeButton contentType="mission" id={m.id} onDone={onRefresh} />
                  )}
