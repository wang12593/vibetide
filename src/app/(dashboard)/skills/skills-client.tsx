...
const categoryLabels: Record<SkillCategory, string> = {
  info_perception: "信息感知",
  info_processing: "信息加工",
  system_interop: "系统互操作",
  automation: "自动化工具",
  communication: "通讯协作",
  multimodal: "多模态处理",
  other: "其他",
...
export function SkillsClient({
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      <PageHeader
        title="技能管理"
        description="管理 数字员工可用的技能库，添加自定义技能或查看内置技能"
        actions={
            ...
            <GlassCard key={skill.id} variant="interactive" padding="none">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-t border-gray-200/50 dark:border-gray-700/50">
                <div className="flex items-center gap-2">
                  ...
                  {skill.visibility === "personal" && skill.type !== "builtin" && (
                    <SkillUpgradeButton
                      skillId={skill.id}
                      onDone={() => {
                        setLocalSkills((prev) =>
                          prev.map((s) =>
                            s.id === skill.id ? { ...s, visibility: "org" as const } : s
                          )
                        );
                        router.refresh();
                      }}
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget?.bindCount
            ? `该技能已被 ${deleteTarget.bindCount} 个数字员工绑定，删除后将自动解绑。确定要删除「${deleteTarget.name}」吗？`
...
function SkillUpgradeButton({
  ...
  async function handleUpgrade() {
    setLoading(true);
    try {
      await upgradeContentVisibility("skill", skillId);
  ...
  return (
    <Button
      variant="ghost"
      size="sm"
      className="text-[11px] h-auto p-0 text-gray-400 hover:text-blue-600 dark:text-gray-500 dark:hover:text-blue-400"
      onClick={(e) => {
        e.stopPropagation();
        handleUpgrade();
      }}
      disabled={loading}
    >
      {loading ? <Loader2 size={10} className="mr-1 animate-spin" /> : <Users size={10} className="mr-0.5" />}
      {loading ? "升级中..." : "升级为组织共享"}
    </Button>
  );
}
